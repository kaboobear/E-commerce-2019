...
router.get('/counts', auth, async (req, res) => {
  ...
  const totalCount = orders.reduce(
    (prev, current) => prev + parseInt(current.cart.count),
    0,
  );
  const summary = orders.reduce(
    (prev, current) => prev + current.cart.total,
    0,
  );
...
router.post('/login', (req, res) => {
  ...
  User.findOne({ mail }).then((user) => {
    if (!user) {
      return res.status(404).json({ mail: 'Почта не найдена' });
    }
    ...
    bcrypt.compare(pass, user.pass).then((isMatch) => {
      if (isMatch) {
        ...
        return res.status(400).json({ pass: 'Введен неправильный пароль' });
    });
  });
});
...
router.post('/editMail', (req, res) => {
  ...
  if (mail === data.user.mail) return res.status(200);

  User.findOne({ mail }).then((user) => {
    if (user) {
      return res.status(400).json({ mail: 'Почта уже используется' });
    }
...
router.post('/editPass', (req, res) => {
  ...
  User.findById(user._id).then((user) => {
    bcrypt.compare(oldPass, user.pass).then((isMatch) => {
      if (isMatch) {
        bcrypt.genSalt(10, (err, salt) => {
        ...
        return res.status(400).json({ oldPass: 'Введен неправильный пароль' });
    });
  });
});
...
router.post('/setDelivery', (req, res) => {
  ...
  User.findByIdAndUpdate(data.user._id, { delivery }, { new: true }).then(
    (user) => {
