...
class ProductItems extends Component {
  ...
  componentDidMount() {
    this.props.getItems(1, 1000, 0, 0, 10000000, 0, 1, '', false);
  }
  ...
  render() {
    ...
    return (
      <div className="admin-products-wrapper">
        <div className="admin-wrap-title">Продукты</div>
        ...
        <div className={`admin-items ${isLoading ? 'loading' : 'notLoading'}`}>
          ...
          {items.map((elem) => (
            <AdminItem elem={elem} key={elem._id} />
          ))}
        </div>
      </div>
    );
...
export default withRouter(
  connect(mapStateToProps, { getItems, deleteItem, setUpdated })(ProductItems),
);
