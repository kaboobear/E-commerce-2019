...
class CartItem extends Component {
  ...
  onDeleteClick = () => {
    this.props.deleteCart(this.props.elem.item._id);
  };
  ...
  minus = (elem) => {
    if (elem.count === 1) {
      this.props.deleteCart(elem.item._id);
    } else {
      this.props.addToCart(elem.item, -1);
      this.setState({ count: this.state.count - 1 });
    }
    ...
    return (
      <div className="item cart-item">
        ...
        <div className="item-title">
          <NavLink to={`item/${elem.item._id}`} className="item-title-text">
            {elem.item.title}
          </NavLink>
        </div>
        ...
        <div className="item-category">
          <img
            ...
            src={`/img/c${elem.item.category}.png`}
            alt=""
          />
        </div>
        ...
        <div className="item-price">
          {elem.item.price} <span className="dollar">грн</span>
        </div>
