...
export const calcCounts = () => (dispatch, getState) => {
  ...
  axios
    .get('/user/counts', tokenConfig(getState))
    .then((res) => {
...
export const editMail = (mail) => (dispatch) => {
  ...
  axios
    .post('/user/editMail', body, config)
    .then((res) => {
      notifyMail();
    ...
    .catch((err) => {
      dispatch(
        returnErrors(err.response.data, err.response.status, 'AUTH_EDIT_FAIL'),
      );
    });
};
...
export const editPass = (data) => (dispatch) => {
  ...
  axios
    .post('/user/editPass', body, config)
    .then((res) => {
      notifyPass();
      dispatch(clearErrors());
...
export const setDelivery = (data) => (dispatch) => {
  ...
  const body = JSON.stringify(data);
  axios
    .post('/user/setDelivery', body, config)
    .then((res) => {
      notifyDelivery();
      dispatch(clearErrors());
